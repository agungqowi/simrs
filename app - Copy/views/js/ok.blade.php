	<script type="text/javascript">
		...
		function pasien_find(val,norm){
            var _url = "";
            if(val ==''){
               _url= "{{ url('rest/rawat_inap_norm') }}"+'/'+norm;
            }
            else{
                ...
                success: function(res){
                    ...
                    if(res == false){
                        ...
                        $.each(res, function (key, data) {
                            ...
                            _html += "<td>"+data.NamaDokter+"</td>";
                            _html += "<td>"+data.NamaParamedis+"</td>";
                            _html += "<td align='right'>"+data.Tarif+"</td>";
            ...
            if(tanggal_tindakan != '' && id_tindakan != '')
            {
                $.ajax({
                    ...
                    success:function(res){
                        ...
                        $('#txt_dokter_tindakan').val('');
                        $('#txt_perawat_tindakan').val('');
        ...
        function do_simpan_rujuk(){
            var id_inap     = $('#id_inap').val();
            var id_ppk      = $('#id_ppk_rujukan').val();
            var nama_ppk    = $('#ppk_rujukan').val();

            var _image  = "{{ url('img/load_gif.gif') }}";

            $('#rujuk_pesan').removeClass('alert-error');
            $('#rujuk_pesan').removeClass('alert-success');
            $('#rujuk_pesan').addClass('alert-error');

            $('#rujuk_pesan').html("<img src='"+_image+"' />")

            if( id_inap == "" || id_inap == '-' ){
                $('#rujuk_pesan').html('Mohon pilih pasien terlebih dahulu');
            }
            else if(id_ppk == "" || id_ppk == '0'){
                $('#rujuk_pesan').html('Mohon pilih rumah sakit rujukan terlebih dahulu')
            }
            else{
                $.ajax({
                    url: "{{ url('unit/simpan_rujuk') }}",
                    type: "POST",
                    data : "id_inap="+id_inap+"&id_ppk="+id_ppk+"&nama_ppk="+nama_ppk,
                    success:function(res){
                        list_rujuk();
                    },
