    <script type="text/javascript">
        ...
        function list_tindakan(opt){
            ...
            target_url = "{{ url('penunjang/list_tindakan/lab') }}";
            $.ajax({
                url: target_url+'/'+val,
                dataType: "json",
                success: function(res){
            ...
            if(tanggal_tindakan != '' && tindakan != '')
            {
                $.ajax({
                    ...
                    data : "noreg="+NoReg+"&id_tindakan="+id_tindakan+"&tanggal_tindakan="+
                            ...
                            "&tindakan="+tindakan+"&gol="+gol+"&jenis_rawat=lab",
                    success:function(res){
                        list_tindakan();
        ...
        function tambah_diagnosa(){
            ...
            $.ajax({
                ...
                data : "noreg="+NoReg+"&id_diagnosa="+id_diagnosa+"&nama="+txt_nama+"&norm="+id_norm+
                        "&short="+_short+"&long="+_long+"&tanggal="+tanggal_diagnosa+"&status="+status_diagnosa+"&jenis_rawat=lab",
                success:function(res){
                    $('#cari_diagnosa').modal('hide');
        ...
        $('#btn_kategori').click(function(){
            var _val = $('#kategori_pemeriksaan').val();
        ...
        function tambahKategori(_val){
            if( _val != '' ){

                if( $('.paket_'+_val).length ){

                }
                else{
                    var _html = "<tr class='paket_"+_val+"'>";
                    _html += "<td colspan='5'><b>"+$("#kategori_pemeriksaan option:selected").text()+"</td>";
                    _html += "</tr>";

                    //$('#table_hasil tbody').append(_html);

                    var target_url = "{{ url('lab/kategori') }}";
                    $.ajax({
                        url: target_url+'/'+_val,
                        dataType: "json",
                        success: function(response){
                            if(response == false)
                                ...
                                $.each(res,function(key,obj){
                                    _html += "<tr class='paket_"+_val+"'>";
                                    _html += "<td>"+obj.nama_periksa+"</td>";
                                    _html += '<td><input type="text" id="hasil_'+obj.id+'" name="hasil['+obj.id+']" /></td>';
                                    _html += "<td>"+obj.nilai_rujukan+"</td>";
                                    _html += "<td>"+obj.satuan;
                                    _html += '<input type="hidden" id="kategori_'+obj.id+'" name="kategori['+obj.id+']" value="'+_val+'"/>';
                                    _html += '</td><td><textarea id="catatan_'+obj.id+'" name="catatan['+obj.id+']"></textarea></td>';
                                    _html += "</tr>";
            ...
            $.ajax({
                ...
                success:function(res){
                    if(res.status == 'ada'){
                        ...
                        var _cat = _res.kategori;

                        var _cats = _cat.split(",");

                        _length = _cats.length;

                        _loop = 0;
                        $.each(_cats,function(number ,obj){
                            $("#kategori_pemeriksaan").val(obj);
                            tambahKategori(obj);
                        });
                ...
                $.ajax({
                    ...
                    success:function(res){
                        if(res.status == 'ada'){
                            ...
                            $.each(_res,function(number ,obj){
                                $('#hasil_'+obj.id_pemeriksaan).val(obj.hasil);
                                $('#catatan_'+obj.id_pemeriksaan).val(obj.catatan);
                            });
                        }
                    }
                });
