...
@section('content')
	<div id="contentwrapper">
		<div class="main_content">
        	...
        	<div class="row-fluid">
	        	<div class="span12">
	        		<h3 class="heading">Daftar Dokter
                        <div style="float:right;" class="">
                            <a href="{{ URL::to('dokter/create') }}" class="btn btn-success"><i class="splashy-contact_grey_add"></i> Tambah Dokter Baru</a>
                        </div>
                    </h3>
	        	</div>
            </div>
            <div class="row-fluid">
                ...
                <?php $success = Session::get('success'); ?>
                    ...
                    @endif
                {{ Datatable::table()->addColumn('ID Dokter','Nama Dokter','NIP','Spesialisasi',
                            'No Telp','Keterangan','')
                        ->setOptions('sPaginationType','bootstrap')
                        ->setUrl(URL::to('dokter/datatable'))->render() }}
                </div>
	        </div>
	   	</div>
	</div>
    ...
    {{ Form::open(array('url' => 'dokter/', 'id' => 'delete_form', 'class' => 'pull-right')) }}
        {{ Form::hidden('_method', 'DELETE') }}
        <input type="hidden" id="url_delete" value="{{ url('dokter') }}">
