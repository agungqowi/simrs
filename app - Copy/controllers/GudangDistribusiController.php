...
class GudangDistribusiController extends \BaseController {

	public $pref = "as";
	public $title = 'Distribusi Gudang';
	public $slug = 'askes';
	/**
	...
	public function index($pref='askes')
	{
		...
		return View::make('gudang.distribusi' , array(
			...
			if($jumlah > 0 || $id_obat == 'NEW'){
				...
				if($tipe == 'edit'){
					...
					foreach($select as $s){
						...
						$obat2 = DB::table('apo_detailobat')->where('kodobat', $s->kodobat )->where('tempat',$tujuan)->first();
						if( isset($obat2->stok) ){
					...
					$check_detail = DB::table('apo_detailobat')->where('kodobat' , $id_obat)->where('tempat' , $tujuan)->first();
					...
					if( isset($check_detail) && !empty($check_detail->id)){
						...
						$data_obat 	= DB::table( 'apo_obat' )->where('kodobat' , $id_obat)->first();
						$update_d 	= DB::table( 'apo_detailobat' )->insert(
								array(
										'stok' => $jumlah , 'kodobat' => $id_obat , 'tempat' => $tujuan ,
										'namaobat' => $data_obat->namaobat , 'kodejenis' => $data_obat->kodejenis ,
										'komposisi' => $data_obat->komposisi , 'satuan' => $data_obat->komposisi ,
										'masa' => $data_obat->masa
								)
							);
					}
					DB::table('apo_distribusi')->insert(
						array(	'kodobat' => $id_obat , 'dari' => 'gudang' , 'ke' => $tujuan ,
								'jumlah'  => $jumlah,
								'tanggal' => $tgl_masuk->format('Y-m-d') , 'keterangan' => 'distribusi' , 
	...
	public function list_transaksi($pref="askes")
	{
		...
		$data = DB::table('apo_distribusi')->join('apo_obat','apo_distribusi.kodobat' ,'=' ,'apo_obat.kodobat')
				->select('apo_distribusi.*' , 'apo_obat.kodejenis' , 'apo_obat.namaobat' , 'apo_obat.komposisi' , 'apo_obat.satuan' , 'apo_obat.stok')
				->where('apo_distribusi.tanggal','=' ,$tanggal)->where('apo_distribusi.keterangan' ,'=' , 'distribusi')->get();
		return $data;
	}
