...
@section('content')
    <div id="contentwrapper">
        <div class="main_content">
            <nav>
                <div id="jCrumbs" class="breadCrumb module">
                    <ul>
                        ...
                        <li>
                            <a href="{{ url('klaim_bpjs/rawat_jalan') }}">Jasa Dokter</a>
                        </li>
                        <li>
                            Rawat Jalan
                        </li>
            ...
            <div class="row-fluid">
                <div class="span12">
                    <h3 class="heading">Input Klaim BPJS Rawat Jalan
                    </h3>
                                    ...
                                    <tr class="{{ $p->NoRegJalan }} {{ $p->NoRM }} {{ $p->Nama }}">
                                        ...
                                        <td>
                                            @if($p->TotalKlaim=='' || empty($p->TotalKlaim))
                                                <?php $p->TotalKlaim = 0; ?>
                                            @endif
                                            <input style="width:80px" class="numeric total-klaim" data-noreg="{{ $p->NoRegJalan }}" required type="text" style="width:100px" name="klaim['{{ $p->NoRegJalan }}']" id="klaim_{{ $p->NoRegJalan }}" value="{{ $p->TotalKlaim }}" />
                                        </td>
                                        <td>{{ $p->GolPasien }}</td>
                                        <td>
                                            @foreach( $helper->getDokterByNoReg($p->NoRegJalan) as $s )
                                                {{ $s->NamaDokter .'<br />'}}
                                            @endforeach
                                        </td>
                                        <td>
