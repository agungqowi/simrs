	<script type="text/javascript">
            ...
            $(document).ready(function(){
                ...
                $('#btn_cetak4').click(function(){
                    $('#print4').html( "Nama : "+$('#txt_nama').val() + "<br />RM : " + $('#no_rm').val() + "<br />TL : " + $('#txt_tanggal_lahir').val()  );
                    printJS('print4', 'html');
                });
                        ...
                        $.ajax({
                            ...
                            success:function(nres){
                                 if(nres.pesan != 'sukses'){
                                    ...
                                    $('#btn_cetak3').attr('target','{{ url("/igd/struk/") }}/'+nres.noreg);
        ...
        function get_history()
        {
            var val = $('#no_rm').val();
            $.ajax({
                url: "{{ url('pasien/history/ugd') }}"+'/'+val,
                dataType: "json",
                success: function(res){
                    ...
                    if(res == false){
                        ...
                        $.each(res, function (key, data) {
                            $('#tbl_history_pasien tbody').append('<tr><td>'+data.NoRegUGD+'</td>'+
                                '<td>'+data.Tanggal+'</td>'+
                                '<td><a href="javascript:void(0)" onclick="hapus_riwayat('+"'"+data.NoRegUGD+"'"+')" class="btn btn-danger">Hapus</a></td>'+'</tr>');
                        });
                    }
                }
            });
