...
class AskesObatController extends \CrudController {
	 ...
	 * @return Response
	 */
	public $title 		= 'Data Obat Apotek';
	public $table 		= 'apo_detailobat';
	public $slug 		= 'askes_obat';
	public $controller 	= 'AskesObatController';
	...
	public function getColumns(){
		...
		$column['stok'] 		= 'Stok Apotek';
		$column['hargabeli'] 		= 'Harga Beli';
		$column['het'] 				= 'HET';
		$column['harga_jual_umum'] 		= array( 'title' => 'Harga Jual Umum' , 'type' => 'currency' );
		$column['harga_jual_asuransi'] 	= array( 'title' => 'Harga Jual Asuransi' , 'type' => 'currency' );
		$column['harga_jual_bpjs'] 		= array( 'title' => 'Harga Jual BPJS' , 'type' => 'currency' );
		$column['harga_jual_pt'] 		= array( 'title' => 'Harga Jual PT' , 'type' => 'currency' );
	...
	public function update($id){
		...
		if( isset($check->id) ){
			DB::table('apo_detailobat')->where('id' , $check->id)->update(array(
				...
				'harga_jual_pt' 		=> Input::get('harga_jual_pt') ,
				'harga_jual_bpjs' 		=> Input::get('harga_jual_bpjs') ,
				'persentase_umum' 		=> Input::get('persentase_umum') , 
				'persentase_asuransi' 	=> Input::get('persentase_asuransi') ,
				'persentase_pt' 		=> Input::get('persentase_pt') ,
				'persentase_bpjs' 		=> Input::get('persentase_bpjs') ,
				'pembulatan_umum' 		=> Input::get('pembulatan_umum') , 
				'pembulatan_asuransi' 	=> Input::get('pembulatan_asuransi') ,
				'pembulatan_pt' 		=> Input::get('pembulatan_pt') ,
				'het' 					=> Input::get('het') ,
				'pembulatan_bpjs' 		=> Input::get('pembulatan_bpjs')
			));
		}
		else{
	...
	public function simpanHarga(){
		...
		if( $flag ){
			...
			foreach ($obat as $key) {
				...
				$jenis = DB::table('apo_jenisobat')->where('id' , $key->kodejenis)->first();
				...
				if( isset($check->id) ){
					...
					DB::table('apo_detailobat')->insert(array(
						'kodobat' 				=> $id , 
						'namaobat'				=> $key->namaobat ,
						'satuan'				=> $key->satuan ,
						'komposisi'				=> $key->komposisi ,
						'kodejenis'				=> $key->kodejenis ,
						'namajenis'				=> $namajenis ,
						'tempat' 				=> 'apotek_askes' , 
						'harga_jual_umum' 		=> $harga_jual_umum , 
						'harga_jual_asuransi' 	=> $harga_jual_asuransi ,
