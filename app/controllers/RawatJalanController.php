...
class RawatJalanController extends \BaseController {
	...
	public function store()
	{
		...
		if ($validator->fails()) {
			$return = array('pesan'=>'Error, masukkan data NoRM, tanggal dan jam masuk');
			echo json_encode($return);
			die();

		} else {

			$noregold 	= Input::get('no_reg_jalan');
			if( $noregold != '0' ){
				$rawat_jalan 				= RawatJalan::find( $noregold );

				$rawat_jalan->IdPoli 		= Input::get('cmb2_poli');
				$rawat_jalan->IdDokter 		= Input::get('cmb2_dokter');
				$rawat_jalan->Keterangan 	= Input::get('txt2_keterangan');
				$rawat_jalan->IdDiag 		= Input::get('id_diagnosa');
				$rawat_jalan->Diagnosis 	= Input::get('diagnosa');

				$rawat_jalan->NoRujukan 	= Input::get('no_rujukan');
				$rawat_jalan->TanggalRujukan= Input::get('tanggal_rujukan');
				$rawat_jalan->IdPPK 		= Input::get('id_ppk_rujukan');
				$rawat_jalan->NamaPPK 		= Input::get('ppk_rujukan');
				...
				$return = array('pesan'=>'sukses' , 'noreg' => $rawat_jalan->NoRegJalan);
			...
			$manual 	= Input::get('new_manual');
			if($pasien_check && $norm != '' ){
			...
			if($check_pasien > 0){
				$return = array('pesan'=>'Gagal menambahkan pasien '.$norm.', Pasien sudah terdaftar di hari yang sama (BPJS)');
				echo json_encode($return);
				die();
			}
			else{
				...
				$biaya_admin 	= 0;
				$biaya_konsul 	= 0;
				$tipe_pasien 	= Input::get('cmb_cara_bayar');
				$otomatis 		= array();
				if( $tipe_pasien == 'BPJS' ){
					$tipe_p = 1;
				}
				...
				if(isset($poli->NamaPoli)){
					...
					if( $tipe_poli == '2' ){
						if(isset($dokter->Spesialisasi)){
							...
							if( $tipe_p == 1 ){
								$otomatis 	= DB::table('tbtindakan')->where('OtomatisIGD' , 1)
								->where('OtomatisBPJS' , 1)
								->get();
							}
							else{
								$otomatis 	= DB::table('tbtindakan')->where('OtomatisIGD' , 1)
								->where('OtomatisUmum' , 1)
								->get();
							}
						...
						if( $tipe_p == 1 ){
								$otomatis 	= DB::table('tbtindakan')->where('Otomatis' , 1)
								->where('OtomatisBPJS' , 1)
								->get();
							}
							else{
						...
						if( $tipe_p == 1 ){
								$otomatis 	= DB::table('tbtindakan')->where('OtomatisSpesialis' , 1)
								->where('OtomatisBPJS' , 1)
								->get();
						}
						else{
				...
				if( count($tindakan) > 0 ){
					foreach($tindakan as $ti){
						...
						$total_tindakan 	= $total_tindakan + floatval( $tarif );
					}
					
				}

				$total 	=	$total_tindakan	+ $biaya_konsul;

				DB::table('tbpasienjalan')->where('IdRegJalan' , $id_reg)->update(
					array(
						'Total'	=> $total
					)
				);
				...
				$return = array('pesan'=>'sukses' , 'noreg' => $no_reg_jalan , 'norm' => $norm);
	...
	public function register()
	{
		...
		$dokter = Dokter::orderBy('NamaDokter' , 'ASC')->where('status' ,'aktif')->get();
	...
	public function pasien()
	{
		...
		return View::make('rawat_jalan.pasien' ,  
			array('poli' => $poli , 'lab' => $lab , 'rad' => $radiologi ,
	...
	public function pasienProses($reg)
	{
		...
		$data 	= DB::table('tbpasienjalan')->where('IdRegJalan' , $reg)->first();

		return View::make('rawat_jalan.pasien' ,  array( 'reg' => $reg , 'poli' => $poli , 'lab' => $lab , 'rad' => $radiologi , 'rawat' => $data , 'dokter' => $dokter , 'perawat' => $perawat ) );
	}
	...
	public function tambah_dokter()
	{
		...
		if($check_dokter > 0){
			...
			if( isset($pasien_jalan->IdPoli)){
				...
				$tipe_pasien 	= $pasien_jalan->CaraBayar;
	...
	{
		$check 	= DB::table('tbpasienjalan')->where('IdRegJalan', '=', $id)->where('StatusBayar' , '0')->first();
		if( isset($check->IdRegJalan) && !empty($check->IdRegJalan) ){
			return '0';
		}
		else{
			return '1';
	...
	public function tambah_poli(){
		$noreg = Input::get('noreg');
		$id_poli = Input::get('id_poli');
		$pasien = DB::table('tbpasienjalan')->where('IdJalan', '=', $noreg)->first();
		...
		if(isset($pasien->NoRegJalan)){
			$update = DB::table('tbpasienjalan')->where('IdJalan', '=', $noreg)->update(
					array( 
						'IdPoli' 	=> $id_poli ,
						'Poli'		=> $poli
					)
			...
			if(isset($jalan->NoRegJalan)){
				...
				$rawat_jalan->jam_daftar = $jalan->jam_daftar;
				$rawat_jalan->IdPoli = $id_poli;

				
				$rawat_jalan->Poli = $nama_poli;
				$rawat_jalan->NoRegJalan = $jalan->NoRegJalan;
				$rawat_jalan->CaraBayar = $jalan->CaraBayar;
				$rawat_jalan->save();
				die('sukses');
			}
			else{
	...
	public function rujuk_ruangan(){
		...
		$tgl_masuk 		= Input::get('tanggal_rujuk');
		if( empty($tgl_masuk) ){

			$tanggal = date('Y-m-d');
		}
		else{
			$tgls 		= explode('/', $tgl_masuk );
			if( isset($tgls[2]) ){
		...
		$jam 		= Input::get('jam_rujuk');
				

		$check 		= DB::table('tbpasieninap')->where('NoRegJalan' , $id_reg)->first();
		if( isset($check->NoRegJalan) ){

			//$check_tindakan 	= DB::table('tbdetailtindakan')->where( 'NoReg' , $check->NoReg )->get();
			$check_tindakan 	= 0;
			if( count($check_tindakan) > 0 ){
				...
				if( count($ruangan) > 0 ){
					foreach ($ruangan as $r) {
						$data_update['Ruangan'] 		= $r->NamaRuangan;
						$data_update['IdRuangan'] 		= $id_ruangan;
						$data_update['Kelas'] 			= $r->IdKelas;
						$data_update['NoKamar'] 		= $r->NoKamar;
						$data_update['TarifRuangan'] 	= $r->Tarif;
						$data_update['BiayaAdm'] 		= $r->Administrasi;
						$data_update['BiayaMakan'] 		= $r->BiayaMakan;
						$data_update['BiayaKamar'] 		= $r->BiayaKamar;
						$data_update['BiayaDokter'] 	= $r->JasaDokter;
						$data_update['BiayaPerawat'] 	= $r->JasaPerawat;

						$nama_ruangan 					= $r->NamaRuangan;
					}
					$up 	= DB::table('tbpasieninap')->where('IdInap' , $check->IdInap)->update(
						$data_update
					);

					$return = array('status'=>'sukses' , 'pesan' => 'Pasien berhasil dirujuk ulang ke '.$nama_ruangan);
				}
				else{
					$return = array('status'=>'gagal' , 'pesan' => 'Gagal merujuk pasien ke rawat inap, masukkan ruangan terlebih dahulu');
				}
			}
			...
			if( isset($data->NoRegJalan)) {		
				...
				if(  $id_ruangan == '-' ||   $id_ruangan =='' ){
					$return = array('status'=>'gagal' , 'pesan' => 'Mohon isi ruangan terlebih dahulu ');
				}
				else{
					$norm 					= $data->NoRM;
					$pasien_masuk  			= new PasienMasuk;
					...
					$pasien_masuk->NoRM 	= $norm;
					$pasien_masuk->TglMasuk = $tanggal;
					$pasien_masuk->JamMasuk = $jam;
	...
	public function list_rawatinap($id=0){
		$data 	= DB::table('tbpasieninap')->where('NoRegJalan' , $id)->orderBy('IdInap','ASC')->get();
		if( count($data) > 0 ){
			$tanggal = $jam = $ruangan 	= $kelas 	= $no = "";
			foreach($data as $d){
				$ruangan 	= $d->Ruangan;
				$kelas 		= $d->Kelas;
				$no 		= $d->NoKamar;
				$tgls 		= explode( '-' , $d->Tanggal );
				$tanggal 	= $tgls[2].'/'.$tgls[1].'/'.$tgls[0];
				$jam 		= $d->Jam;
			}
			$return = array('status' => true, 'ruangan' => $ruangan , 'pesan' => 'Pasien telah dirujuk rawat inap ke Ruangan '.$ruangan.' '.$kelas.' No Kamar :'.$no.' Pada '.$tanggal.' '.$jam);
	...
	public function list_pasien(){
		...
		$text 		= Input::get('text');
		if( empty($text) || $text == '' ){
			$data 		= DB::table('tbpasienjalan')->join('tbpasien' , 'tbpasienjalan.NoRM' , '=' , 'tbpasien.NoRM')->
					where('Tanggal' , $tanggal)->orderBy('NoRegJalan' , 'DESC')->get();
		}
		else{
			$data 		= DB::table('tbpasienjalan')->join('tbpasien' , 'tbpasienjalan.NoRM' , '=' , 'tbpasien.NoRM')
					->where('Tanggal' , $tanggal)->where(function($query) use ($text)
		            {
		                $query->orWhere('tbpasien.NoRM', 'LIKE', $text.'%')
		                      ->orWhere('tbpasien.Nama', 'LIKE', '%'.$text.'%');
		            })
					->orderBy('NoRegJalan' , 'DESC')->get();
		}
	...
	public function selesaiPeriksa(){
		...
		$jam_periksa 		= Input::get('jam_periksa');
		$selesai_periksa 	= Input::get('selesai_periksa');

		$id_reg				= Input::get('id');

		$data 					= array();
		$data['jam_selesai'] 	= $jam_selesai;
		$data['jam_periksa'] 	= $jam_periksa;
		$data['status'] 		= $selesai_periksa;

		$return 	= array();

		if( $selesai_periksa == '5' ){
			$data['poli_rujuk']	= Input::get('poli_lain');
		}

		if( $selesai_periksa != '0' ){
			$ket 	= DB::table('status_keluar')->where('status' ,$selesai_periksa )->first();
			if( isset($ket->keterangan) ){
				$keterangan_pulang = $ket->keterangan;
			}
			else{
				$keterangan_pulang = '';
			}
		...
		$data['keterangan_pulang'] 	= $keterangan_pulang;

		$update_selesai		= DB::table('tbpasienjalan')->where('IdRegJalan' , $id_reg)->update(
									$data
								);
		$return['status']	= 1;
		$return['pesan']	= 'Update data berhasil';
