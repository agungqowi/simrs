...
class GudangHibahController extends \BaseController {

	public $pref = "as";
	public $title = 'Gudang Obat';
	public $slug = 'askes';
	/**
	...
	public function index($pref='askes')
	{
		...
		return View::make('gudang.hibah' , array(
			...
			if($jumlah > 0 || $id_obat == 'NEW'){
				...
				if($tipe == 'edit'){
					...
					DB::table('apo_pembelian')->insert(
						array(	'kodobat' => $id_obat , 'hargabeli' => 0 , 
								'jumlah'  => $jumlah,
								'tanggal_beli' => $tgl_masuk->format('Y-m-d') , 'ppn' => 0 ,
								'diskon' => Input::get('diskon') , 'jenis_pembayaran' => Input::get('jenis_pembayaran') ,
								'kodesupp' => Input::get('kode_supplier') , 'tanggal_expire' => $masa)
					);
