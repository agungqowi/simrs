...
class ReportObatController extends \ReportController {
	...
	public function penjualanObat_view($pref='askes'){
		...
		$data = DB::table('apo_penjualan_detail')
				...
				->select('apo_penjualan_detail.*' , DB::raw('DATE_FORMAT(TanggalResep, "%d/%m/%Y") AS tgl') , 'apo_penjualan.JenisRawat' ,'apo_penjualan.NoRM','apo_penjualan.Nama')
				->orderBy('apo_penjualan_detail.id_penjualan' , 'ASC')
	...
	public function penjualanPerObat($pref='askes')
	{	
		$this->title = 'Apotek';
		if($pref == 'apotek_askes'){
			$this->title = 'Apotek Askes';
	...
	public function penjualanPerObat_view($pref='askes'){
		$this->pref = "apo_";
		$this->title = 'Apotek';
		$this->slug = $pref;
		...
		if( $obat != '' ){
			$data 	= $data->where('apo_penjualan_detail.kodobat' , $obat);
		}

		$data 	= $data->select('apo_penjualan_detail.*' , 
								DB::raw('DATE_FORMAT(TanggalResep, "%d/%m/%Y") AS tgl') , 
								'apo_penjualan.JenisRawat' ,'apo_penjualan.NoRM','apo_penjualan.Nama')
				->orderBy('apo_penjualan_detail.IdObat' , 'ASC')
				->orderBy('apo_penjualan_detail.TanggalResep' , 'ASC')
				->get();

		return View::make('report.obat.penjualan_perobat_view' , 
			array(
				'data' => $data,
				'dari_tanggal' => Input::get('dari_tanggal') ,
				'sampai_tanggal' => Input::get('sampai_tanggal'),
