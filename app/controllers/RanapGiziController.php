...
class RanapGiziController extends \CrudController {
	...
	public function index($tanggal=""){
		...
		$pasien 	= $pasien->where('Tanggal','<=',$tanggal);
		$pasien 	= $pasien->where(
							function($q){
								$q->orWhere(function($query)
					            {
					                $query->where('TanggalPulang', '>=', $this->tanggal );
					            });
					            $q->orWhere(function($query)
					            {
					                $query->where('TanggalPulang','=','0000-00-00');
					            });
					            $q->orWhere(function($query)
					            {
					                $query->where('TanggalPulang','=','9999-12-31');
					            });
							}
						);

		//where('TanggalPulang','>=',$tanggal);
		$pasien 	= $pasien->get();
	...
	public function orderGizi(){
		...
		if( count($tgls) > 1 ){
			...
			if( isset($check->id) && $check->id > 0 ){
				...
				if( isset($check_tanggal) && $check_tanggal->IdInap > 0 ){
					if( $check_tanggal->TanggalPulang == '0000-00-00' || $check_tanggal->TanggalPulang == '9999-12-31'){
						$insert 		= DB::table('pasien_makan')->insert($data);
						$return 		= array('status' => 'success' , 'pesan' => 'Order gizi berhasil di tambah'  );
					}
					else{
						if( $check_tanggal->TanggalPulang >= $tanggal ){
							$insert 		= DB::table('pasien_makan')->insert($data);
							$return 		= array('status' => 'success' , 'pesan' => 'Order gizi berhasil di tambah'  );
						}
						else{
