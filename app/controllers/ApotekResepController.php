...
class ApotekResepController extends \CrudController {
	...
	public function tambah_resep($pref='askes')
	{
		$this->setPref($pref);
		$rules = array(
		...
		if ($validator->fails()) {
			...
			if( $stok >=  $jumlah){
				...
				$data_penjualan = array(
										'tanggal_resep' => $resep->format('Y-m-d') ,
										...
										'IdReg'				=> Input::get('id_reg') ,

				);	
				if( $id_resep == 0 ){
					$jenis_rawat 	= Input::get('jenis_rawat');
					if( $jenis_rawat == 'RJ' ){
						$check_d	= DB::table('apo_resep')->where('IdReg' , Input::get('id_reg') )->first();
						if( isset($check_d->id) ){
							$id_resep 	= $check_d->id;
						}
						else{
							$id_resep 	= DB::table('apo_resep')->insertGetId($data_penjualan);
						}
					}
					else{
						$id_resep 	= DB::table('apo_resep')->insertGetId($data_penjualan);
				...
				if( isset($stoks->stok) && !empty($stoks->stok) ){
					...
					if( $new_stok >= 0 ){
						$check_obat		= DB::table('apo_resep_detail')
						...
						$check_obat		= DB::table('apo_resep_detail')
											...
											->first();
						if( isset($check_obat->id) ){
							$detail  	= DetailResep::find($check_obat->id);
							$jumlah  	= intVal( Input::get('jumlah') ) + intVal($check_obat->Jumlah);
							$total 		= $jumlah * floatval( Input::get('harga') );
						}
						else{
						...
						$detail->Penggunaan		= Input::get('penggunaan');
						...
						$pesan = 'Stok tidak mencukupi (1)';
					}
				}
				else{
					$pesan = 'Stok tidak mencukupi (2)';
				}
				
			}
			else{
				$pesan = 'Stok tidak mencukupi (3)';
			}

			echo json_encode( array( 'pesan' => $pesan , 'id_resep' => $id_resep ) );
		}
	...
	{
		$this->setPref($pref);
		$data = DB::table('apo_resep_detail')->where('id_resep','=',$id)
		->select('*' , DB::raw('DATE_FORMAT(TanggalResep, "%d/%m/%Y") AS tgl'))->orderby('TanggalResep','desc')->get();
		return $data;
	}

	public function list_transaksi_byreg($pref,$id)
	{
		$this->setPref($pref);
		$return	= false;
		$data 	= DB::table('apo_resep')->where('IdReg',$id)->first();
		if( isset($data->id) ){
			$return 	= DB::table('apo_resep_detail')->where('id_resep','=',$data->id)
						->select('*' , DB::raw('DATE_FORMAT(TanggalResep, "%d/%m/%Y") AS tgl'))
						->orderby('TanggalResep','desc')->get();
		}
		
		return $return;
	}

	public function list_resep($pref,$id)
	{
		$this->setPref($pref);
		$data = DB::table('apo_resep')->where('id_reg','=',$id)
		->select('*' , DB::raw('DATE_FORMAT(TanggalResep, "%d/%m/%Y") AS tgl'))->orderby('TanggalResep','desc')->get();
		return $data;
	}
	...
	public function check_resep($pref = 'askes')
	{
		$jumlah 		= Input::get('jumlah');
		$kodeobat 		= Input::get('id_obat');
		$id_resep 		= Input::get('id_resep');
		$data 		= DB::table('apo_resep_detail')->where('id_resep','=',$id_resep)
	...
	public function edit_transaksi($pref='askes')
	{
		...
		if($IdObat_asli == $IdObat){
			...
			$upddetailobat	 = 0;
			$select = DB::table('apo_penjualan_detail')->where('id' , '=' , $IdResep)->get();
			foreach($select as $s){
				$j 			= $s->Jumlah;
				$stoks 		= DB::table('apo_detailobat')->where('kodobat' , $IdObat)->first();
				if( isset($stoks->stok) ){
					$stok_awal 	= $stoks->stok;
					...
					if( $new_stok > 0 ){
						...
						$upddetailobat = DB::table('apo_penjualan_detail')->where('id' , '=' , $IdResep)
							->update(array(
								'IdObat' => $IdObat,
								'NamaObat' => $NamaObat,
								'Harga' => $Harga,
			...
			if($upddetailobat){
				$penjualan 		= DB::table('apo_penjualan_detail')->where('id',$IdResep)->first();
			...
			if($upddetailobat)
				...
				echo '(2) Proses Update Gagal, Silahkan Ulangi Lagi.';
		}
	}
	...
	public function cetakEtiket($id_penjualan){
		$pdf = new TCPDF('L', PDF_UNIT, array(75, 50), true, 'UTF-8', false);
		//$pdf = new TCPDF();
		//set the logo
		...
		if( count($list_obat) > 0 ){
			...
			foreach($list_obat as $l){
				...
				if( $cetak ){
					...
					$tgla 		= explode('-' , $l->TanggalResep);
					$tglbeli 	= $tgla[2].'/'.$tgla[1].'/'.$tgla[0];
					...
					if( $l->Tipe == 'P' ){
						...
						$html .= "<br />Obat Racikan";
					}
					
					$pdf->writeHTML($html, true, false, true, false, '');

					$pdf->SetFont('helvetica', '', 11);
					$html = "<div>".$l->Dosis." ".$l->Penggunaan."</div>";
	...
	*/	
	...
	function update_ujr(){
		...
		$check 			= DB::table('apo_penjualan')->where('id',$id_penjualan)->first();

		if( isset($check->id) ){

			$subtotal 		= $check->subtotal;
			$total 			= floatval($subtotal) + floatval($ujr);
			$update 		= DB::table('apo_penjualan')->where('id',$id_penjualan)->update( 
				array( 'ujr' => $ujr  , 'total' => $total )
			);
		}
	...
	function check_counter(){
		...
		$check = DB::table('apo_penjualan')->where('JenisRawat' , $jenis_rawat)
		->where('tanggal_transaksi' , $today)
		->get();

		$count 	= count($check) + 1;

		echo $tipe."<br /><br />".$count;
		die();
	}
