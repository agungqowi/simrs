...
class DashboardController extends \BaseController {
	...
	public function index()
	{
		if(Auth::check()) {
			...
			return View::make('dashboard.general', 
					array('user' => $user , 'ruangan' => $ruangan, 'pasien_all' => $pasien_all,
	...
	public function pasien(){
		if(Auth::check()) {
			$user = Auth::user();
			$today = date('Y-m-d');
			$pasien 			= Pasien::where('NoRM' , '!=' , '');
			$pasien_all 		= $pasien->count();
			$pasien_laki 		= $pasien->where('Jkel' ,'=' , 'L')->count();
			$pasien_perempuan 	= intval($pasien_all) - intval($pasien_laki);
			$pasien_bpjs 		= Pasien::where('NoRM' , '!=' , '')->where('GolPasien' ,'=' , 'BPJS')->count();
			$pasien_umum 		= Pasien::where('NoRM' , '!=' , '')->where('GolPasien' ,'=' , 'UMUM')->count();
			$pasien_penjamin 	= intval($pasien_all) - intval($pasien_bpjs) - intval($pasien_umum);  
			$pasien_inap 		= DB::table('tbpasieninap')->where('Tanggal' , '=' , $today)->count();
			$pasien_jalan 		= DB::table('tbpasienjalan')->where('Tanggal' , '=' , $today)->count();
			...
			return View::make('dashboard.pasien', 
					array(
							'user' => $user ,  
							'pasien_all' => $pasien_all,
							'pasien_laki' => $pasien_laki,
							'pasien_perempuan' => $pasien_perempuan,
							'pasien_bpjs' => $pasien_bpjs , 
							'pasien_umum' => $pasien_umum , 
							'pasien_penjamin' => $pasien_penjamin , 
							'pasien_inap' => $pasien_inap , 
							'pasien_jalan' => $pasien_jalan ,
				'menu_d' => $menu_d
			) );
		}
		else{
