...
class RestController extends \BaseController {
	...
	public function pasienByNik($id='')
	{
		if($id==''){
			echo 'false';
		}
		else{
			$pasien = Pasien::where('NoKTP', '=', $id)->take(10)->get()->toJson();;
			echo($pasien);
		}
	}

	public function pasienByKartu($id='')
	{
		if($id==''){
			echo 'false';
		}
		else{
			$pasien = Pasien::where('NoBPJS', '=', $id)->take(10)->get()->toJson();;
			echo($pasien);
		}
	}
	...
	public function kelurahan(){
		...
		return Datatable::query($dokter)
        	...
        	{
        		$model->desa_name 	= str_replace("'", "", $model->desa_name);        		
        		$model->kecamatan_name 	= str_replace("'", "", $model->kecamatan_name);
        		$model->kabupaten_name 	= str_replace("'", "", $model->kabupaten_name);
        		$model->provinsi_name 	= str_replace("'", "", $model->provinsi_name);
